...
using System.Linq;
namespace SAM.API {
  public enum ClientInitializeFailure : byte { Unknown = 0, GetInstallPath, Load, CreateSteamClient, CreateSteamPipe, ConnectToGlobalUser, AppIdMismatch, }
  ...
  public class Client : IDisposable {
    ...
    public void RunCallbacks(bool server) {
      ...
      while (Steam.GetCallback(_Pipe, out message, out _)) {
        ...
        foreach (ICallback callback in _Callbacks.Where(candidate => candidate.Id == callbackId && candidate.IsServer == server)) {
